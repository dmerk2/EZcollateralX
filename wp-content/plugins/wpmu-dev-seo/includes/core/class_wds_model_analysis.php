...
class Smartcrawl_Model_Analysis extends Smartcrawl_Model {
	...
	public function get_post_data( $data_type ) {
		$data = array();
		if ( ! in_array( $data_type, $this->get_known_data_types() ) ) { return false; }
	...
	public function get_readability_levels_map() {
		...
		$map_flesch = array(
			$very_easy => array( 'min' => -10, 'max' => 0.9 ),
			$easy => array( 'min' => 1, 'max' => 1.9 ),
			$fairly_easy => array( 'min' => 2, 'max' => 2.9 ),
			$plain_english => array( 'min' => 3, 'max' => 4.9 ),
			$fairly_difficult => array( 'min' => 5, 'max' => 5.9 ),
			$difficult => array( 'min' => 6, 'max' => 6.9 ),
			$confusing => array( 'min' => 7, 'max' => 15 ),
		);

		$map_kincaid = array(
			$very_easy => array( 'min' => 90, 'max' => 100 ),
			$easy => array( 'min' => 80, 'max' => 89.9 ),
			$fairly_easy => array( 'min' => 70, 'max' => 79.9 ),
			$plain_english => array( 'min' => 60, 'max' => 69.9 ),
			$fairly_difficult => array( 'min' => 50, 'max' => 59.9 ),
			$difficult => array( 'min' => 30, 'max' => 49.9 ),
			$confusing => array( 'min' => 0, 'max' => 29.9 ),
		);
		return Smartcrawl_String::READABILITY_FLESCH === Smartcrawl_String::get_readability_strategy()
			? $map_flesch
			: $map_kincaid
	...
	public function get_readability_level() {
		...
		foreach ( $map as $label => $lvl ) {
			if ( ! is_array( $lvl ) || ! isset( $lvl['min'] ) || ! isset( $lvl['max'] ) ) { continue; }
	...
	public function is_readable() {
		...
		if ( Smartcrawl_Checks::is_readability_ignored( $this->_post_id ) ) { return true; }

		$data = $this->get_post_data( self::DATA_READABILITY );
		if ( empty( $data ) || ! is_array( $data ) || empty( $data['is_readable'] ) ) { return $this->update_readability_data(); }
	...
	public function update_readability_data() {
		...
		if ( false === $raw_result ) { $error = ! empty( $error ) ? $error : __( 'Error calculating readability', 'wds' ); } else { $result = $this->normalize_readability_score( $raw_result ); }
	...
	public function get_kincaid_readability_state( $readability_score, $readability_ignored ) {
		...
		if ( Smartcrawl_String::get_readability_strategy() == Smartcrawl_String::READABILITY_KINCAID ) {
			if ( 0 == $readability_score || $readability_ignored ) {
	 ...
	 * @return array
	 */
	function get_overall_seo_analysis() {
		...
		foreach ( $post_types as $type ) {
			...
			foreach ( $posts as $post ) {
				...
				$has_seo_data = $this->has_post_data( Smartcrawl_Model_Analysis::DATA_ANALYSIS );
				if ( $has_seo_data ) {

					$checked++;
					$checked_in_type++;

					$seo_data = $this->get_post_data( Smartcrawl_Model_Analysis::DATA_ANALYSIS );
	 ...
	 * @return array
	 */
	function get_overall_readability_analysis() {
		$overall_analysis = array();
		$checked = 0;
		$passed = 0;
		$post_types = $this->_get_post_types_for_overview();
		...
		foreach ( $post_types as $type ) {
			...
			foreach ( $posts as $post ) {
				$this->set_post_id( $post->ID );
				$data_available = $this->has_post_data( Smartcrawl_Model_Analysis::DATA_READABILITY );
				if ( $data_available ) {

					$data = $this->get_post_data( Smartcrawl_Model_Analysis::DATA_READABILITY );
